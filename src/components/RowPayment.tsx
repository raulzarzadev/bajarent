	...
	return (
		...
		>
			<View style={{ width: 80 }}>
				...
				<Text>{!!item?.orderFolio && `${item?.orderFolio || ''}-${item?.orderNote || ''}`}</Text>
				<Text numberOfLines={2}>{item?.orderName || ''}</Text>
			</View>
			<DateCell date={item?.createdAt} />
			<View style={{ width: 80 }}>
				<Text numberOfLines={1} style={{ textTransform: 'capitalize', textAlign: 'center' }}>
          ...
          <PaymentVerify payment={item} onVerified={onVerified} />
				...
				<CurrencyAmount amount={isRetirement ? -1 * item?.amount : item?.amount} />
				<Text style={[gStyles.helper, gStyles.tCenter]}>{item?.canceled && 'Cancelado'}</Text>
			</View>
