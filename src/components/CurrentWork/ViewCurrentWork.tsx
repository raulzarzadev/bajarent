...
const ViewCurrentWork = (props?: ViewCurrentWorkProps) => {
	...
	const [currentUpdates, setCurrentUpdates] = useState<CurrentWorkTypeWithOrderAndCustomerData[]>(
		[]
	)

	const [filteredUpdates, setFilteredUpdates] = useState<CurrentWorkTypeWithOrderAndCustomerData[]>(
		[]
	)
		...
		(workType: CurrentWorkView) => {
			...
			if (workType === 'personal') {
				...
				const workOrdersBySections = mySections.reduce((acc, section) => {
					const sectionOrders = orders.filter(o => o.assignToSection === section)
				...
				const sectionOrders = Object.values(workOrdersBySections).flat() as OrderType[]
	...
	const handleSetSelectedSection = useCallback(
		(sectionId: string) => {
			if (selectedSection === sectionId) {
				...
				const sectionOrders = orders.filter(o => o.assignToSection === sectionId)
				const sectionPayments = todayPayments.filter(p =>
					sectionOrders.some(o => o.id === p.orderId)
				)
	...
	return (
		<View style={[gStyles.container]}>
			...
			>
				<Text
					style={[workType === 'sections' && gStyles.tBold, DISABLED_SWITCH && { opacity: 0.5 }]}
				>
					Areas{' '}
				</Text>
				...
				/>
				<Text
					style={[workType === 'personal' && gStyles.tBold, DISABLED_SWITCH && { opacity: 0.5 }]}
				>
					Personal
				</Text>
			</View>
			...
			<BadgeListSectionsE
				sections={sectionsWithUpdates.map(sectionId => sections.find(s => s.id === sectionId))}
					...
					>
						...
						{!!update?.customer && (
							<>
								...
								<Text style={[{ marginRight: 4 }, gStyles.helper]}>{update?.customer?.name}</Text>
							</>
						)}
						...
						{update?.order?.assignToSection && (
							<Text style={[gStyles.helper, gStyles.tBold]}>
								{sections.find(section => section.id === update.order.assignToSection)?.name ||
									update.order.assignToSection}
							</Text>
						)}
					</View>
...
export const getSectionsWorks = ({
	...
	const res = updates.filter(update => sections.includes(update?.details?.sectionId))
