	...
	return (
		<View style={gStyles.container}>
			<View style={{ marginVertical: 16 }}>
				...
				{employee.phone && (
					<Text style={[gStyles.p, { textAlign: 'center' }]}>{employee.phone}</Text>
				)}
				{employee.email && (
					<Text style={[gStyles.p, { textAlign: 'center' }]}>{employee.email}</Text>
				)}

				<Text style={gStyles.h3}>{employee.position}</Text>
			</View>

			<StaffPermissionsE staff={employee} storeId={storeId} />
			...
			>
				{({ handleSubmit, isSubmitting, values }) => {
					//TODO: remove this hooks becouse its inside formik
					...
					const sortCustomerPermissions = (a, b) =>
						values?.permissions?.customers?.[b] - values.permissions?.customers?.[a]
					...
					return (
						<View>
							...
							<Button disabled={isSubmitting} label="Guardar" onPress={() => handleSubmit()} />
						</View>
					)
				}}
			</Formik>
