	...
	return (
		...
		>
			...
			>
				...
				>
					{item.status === 'rented' && (
						<ItemIcon icon="home" iconColor={colors.green} badgeColor={colors.transparent} />
					)}
					{item.status === 'pickedUp' && (
						<ItemIcon icon="truck" iconColor={colors.darkBlue} badgeColor={colors.transparent} />
					)}
					{item?.needFix && (
						<ItemIcon icon="wrench" iconColor={colors.red} badgeColor={colors.transparent} />
					)}
				</View>
			</View>

			<Text style={[gStyles.tCenter, gStyles.helper]}>{item.categoryName} </Text>
			<Text numberOfLines={2} style={[gStyles.h1]}>
				{item.number}
			</Text>
			{showSerialNumber && <Text style={[gStyles.tCenter, gStyles.helper]}>{item.serial}</Text>}
			{showAssignedSection && <Text style={[gStyles.tCenter]}>{sectionName}</Text>}
