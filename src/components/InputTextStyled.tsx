	...
	(
		...
		return (
			<View style={containerStyle}>
				...
				<View style={[baseStyle.inputStyle, disabled && { opacity: 0.5 }, props.style]}>
					<TextInput
						ref={ref}
						{...props}
						...
						onChangeText={text => {
							...
							if (type === 'number') {
								...
								if (decimalPointIndex !== -1) {
									const beforeDecimalPoint = numericText.slice(0, decimalPointIndex + 1)
									const afterDecimalPoint = numericText.slice(decimalPointIndex + 1)
									const sanitizedAfterDecimalPoint = afterDecimalPoint.replace(/\./g, '')
					...
					>
						...
						{leftIcon === 'search' && <Icon icon="search" size={24} color={theme.primary} />}
						{leftIcon === 'close' && <Icon icon="close" size={24} color={theme.error} />}
					</Pressable>
				</View>
