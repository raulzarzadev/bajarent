...
import { type FormOrderFields, mutableFormOrderFields } from './FormOrder'
...
const FormOrdersConfig = ({ onSubmit, defaultValues }: FormOrdersConfigProps) => {
				...
				{({ handleSubmit, values, dirty }) => (
					<View>
						...
						<Text style={gStyles.helper}>
							Te permitira crear diferentes tipos de ordenes según las necesidades de tu negocio
						</Text>
						...
						{Object.keys(values?.orderTypes || {}).map(type => {
							...
							return (
								<View key={type}>
									<Text style={[gStyles.h2, { textTransform: 'capitalize', marginTop: gSpace(2) }]}>
										{dictionary(type)}
									</Text>
