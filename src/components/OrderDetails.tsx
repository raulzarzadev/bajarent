...
const RentItemsInfo = ({ order }: { order: Partial<OrderType> }) => {
	...
	return (
		<View>
			...
			{items?.map(item => (
				<RowOrderItem item={item} key={item.id} order={order} itemId={item.id} />
			))}
		</View>
	)
}
...
export const OrderDates = ({
	...
	return (
		<>
			...
			>
				{!!scheduledAt && <DateCell label="Programada" date={scheduledAt} showTime labelBold />}
				{!!startedAt && <DateCell label="Iniciado" date={startedAt} showTime labelBold />}
				{!!expireAt && (
					<DateCell
						label="Vence"
						date={expireAt}
		...
		<View>
			<ErrorBoundary componentName="OrderPayments">
				...
				>
					...
					>
						<Text style={[gStyles.h3, { marginRight: 8, marginBottom: 0 }]}>Pagos</Text>
						<ModalPayment
							orderId={orderId}
							storeId={storeId}
					...
					{payments.sort(sortByCreatedAt)?.map(payment => (
						...
						>
							...
							<Text style={{ marginHorizontal: 4 }}>{dictionary(payment.method)}</Text>
							<CurrencyAmount style={gStyles.tBold} amount={payment.amount} />
							{payment?.canceled && <Text style={{ marginHorizontal: 4 }}>Cancelado</Text>}
					...
					<Button
						...
						variant="ghost"
						disabled={payments?.length > MAX_PAYMENTS_COUNT || payments?.length < paymentsCount}
						onPress={() => {
...
export const OrderMetadata = ({ order }: { order: Partial<OrderType> }) => {
	...
	return (
		<View>
			<SpanMetadata createdAt={order?.createdAt} createdBy={order?.createdBy} id={order?.id} />
			<View
				style={{
					flexDirection: 'row',
