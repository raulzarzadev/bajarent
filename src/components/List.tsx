...
function MyList<T extends { id: string }>({
	...
	useEffect(() => {
		;(async () => {
			...
			if (storedPinnedRows) {
				...
				const validItems = items?.filter(id => data?.find(row => row?.id === id))
		...
		if (pinnedRows?.length && pinRows) {
			getPinnedRows(pinnedRows).then(orderRowsData => {
				const dataSortedAsInLocalStorage = pinnedRows?.map(orderId => {
					const orderDataFromRedux = data.find(order => order.id === orderId)
					if (orderDataFromRedux) return orderDataFromRedux
					return orderRowsData.find(order => order.id === orderId)
				})
				// here should  get customer data for pinned rows where data is not in redux
	...
	const handleSelectThisPage = value => {
		...
		const newSelectedRows = sortedData.slice(startIndex, endIndex).map(row => row?.id)

		if (value) {
			setSelectedRows([...selectedRows, ...newSelectedRows])
		} else {
			setSelectedRows(selectedRows.filter(row => !newSelectedRows.includes(row)))
		}
	}
		...
		<ScrollView style={{ flex: 1 }}>
			...
			>
				...
				{multiSelect && (
					...
					>
						<InputCheckbox value={selectAll} setValue={handleSelectAll} label="Todas" />
						<InputCheckbox
				...
				{loading ? (
					...
					<SectionList
						...
						sections={[
							...
							{
								title: 'Todas',
								data: slicedData.filter(o => !pinnedRowsData.some(p => p.id === o.id))
							}
						]}
								...
								>
									...
									<Pressable
										style={{
											flex: 1,
											flexDirection: 'row',
											backgroundColor: _pressedRow === item?.id ? '#00000020' : 'transparent'
										}}
										onPress={() => {
											_setPressedRow(item?.id)
											if (multiSelect) {
...
const PinButton = ({
	...
	return (
		...
		>
			<Icon icon={unpin ? 'unPin' : 'pin'} color={unpin ? theme.error : theme.primary} size={22} />
		</Pressable>
	)
}
