...
import type { TimeType } from '../../types/PriceType'
...
const AddExtendExpire = ({ orderId, storeId }: { orderId: string; storeId: string }) => {
	...
	const handleExtend = async () => {
		...
		await onComment({
			orderId,
			storeId,
			content: `Extendio por ${translateTime(`${count} ${unit}`)}. Motivo: ${reason}`,
			type: 'comment'
		})
			.then(console.log)
			.catch(console.error)
				...
				<View style={{ flexDirection: 'column' }}>
					<View style={{ marginVertical: 8 }}>
						...
						<View style={{ marginVertical: 12 }}>
							...
							<Text style={{ textAlign: 'center' }}>{` ${newExpireDate()}`}</Text>
						</View>
						<InputTextStyled placeholder="Motivo" onChangeText={setReason} value={reason} />
					</View>
					<View style={{ marginVertical: 8 }}>
						<Button
							disabled={disabled}
