...
const ModalPickupOrder = ({ modal, order }: { modal: ReturnModal; order: Partial<OrderType> }) => {
	...
	const getCategoryId = category => {
		return categories.find(cat => cat.name === category || cat.id === category)?.id
	}
		...
		items.forEach(async item => {
			const res = await onPickUpItem({
				...
				.then(async (res: FormattedResponse) => {
					if (!res?.ok && res?.res?.code === 'not-found' && shouldCreateItemIfNotExists) {
	...
	return (
		<View>
			<StyledModal {...modal}>
				<TextInfo text="Asegurate de que recoges el siguiente articulo:" defaultVisible />
