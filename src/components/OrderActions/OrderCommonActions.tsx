...
const OrderCommonActions = ({
	...
	const buttons = [
		...
		canAssign && <ModalAssignOrder orderId={orderId} section={order?.assignToSection} />,
			...
			>
				...
				<View style={{ marginVertical: 8 }}>
					<InputTextStyled
						...
						helperTextColor={
							cancelledReason.length < CANCELED_REASON_MIN_LENGTH ? 'error' : 'black'
						}
						helperText={`${
							cancelledReason.length < CANCELED_REASON_MIN_LENGTH ? 'Mínimo 10 caracteres' : ''
						}`}
	...
	return (
		<View>
			...
			>
				...
				<View style={{ padding: 4, width: '50%' }}>
					<ButtonChangeOrderCustomer orderId={order?.id} customerId={order.customerId} />
				</View>
				{/* To fix las element */}
				<View style={{ flex: 1 }} />
			</View>
