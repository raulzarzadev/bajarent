...
export const OrderPDF = () => {
			...
			>
				...
				>
					...
					{!!order?.repairingAt && (
						<Text style={{ fontSize: 14, color: '#666', textAlign: 'center' }}>
							Fecha de inicio: {dateFormat(asDate(order?.repairingAt), 'EE dd/MMM/yy HH:mm')}
						</Text>
					)}
					{!!order?.deliveredAt && (
						<Text style={{ fontSize: 14, color: '#666', textAlign: 'center' }}>
							Fecha de entrega: {dateFormat(asDate(order?.deliveredAt), 'EE dd/MMM/yy HH:mm')}
						</Text>
					)}
				</View>
				...
				<View style={{ marginBottom: 20, width: '100%' }}>
					...
					>
						...
						<Text style={[{ fontSize: 12, marginBottom: 5 }, gStyles.tBold]}>Contactos</Text>
  ...
  //@ts-ignore  */}
						{order?.quotes?.map((item, index) => (
							...
							>
								<Text style={{ fontSize: 16, marginBottom: 5 }}>Producto: {item.description}</Text>
								<Text style={{ fontSize: 16, marginBottom: 5 }}>
				...
				<View style={{ width: '100%', alignItems: 'center' }}>
					{order?.contractSignature?.accept ? (
						...
						<Text style={{ fontSize: 16, marginBottom: 5 }}>No aceptada ❌</Text>
					)}
				</View>
			</View>

			<Button onPress={toPDF} label="Descargar PDF" icon="download" fullWidth={true} />
			<Button
				onPress={() => {
