...
export default function ScreenMessages() {
		...
		if (target === 'expired') {
			const selectedOrders = orders?.filter(
				order =>
					...
					isBefore(endDate(asDate(order.expireAt)), subDays(endDate(new Date()), EXPIRED_DAYS))
				//&& !order.expiresToday
			)
	...
	const canSendMessages = permissions?.store?.canSendMessages || permissions?.isAdmin
		...
		return (
			<View>
				...
				<Text style={[gStyles.helper, { textAlign: 'center' }]}>
					Habilitalo en la configuración de la tienda y agrega los datos necesarios para poder
					enviar mensajes de forma autamatica
				</Text>
			</View>
		)

	if (!canSendMessages) {
		return <Text style={gStyles.h2}>No tienes permisos para enviar mensajes</Text>
	}
	...
	return (
		<ScrollView>
			...
			/>
			<Text style={gStyles.h2}>2. Selecciona las ordenes que recibiran este mensaje</Text>
			<InputRadios
				layout="row"
				value={target}
				setValue={val => {
			...
			{selectedOrders?.length === 0 && (
				<Text style={[gStyles.helper, { textAlign: 'center', marginVertical: 12 }]}>
					...
					rowSideButtons={[
						{
							label: 'Eliminar',
							onPress: rowId => {
								const clearOrders = selectedOrders.filter(order => order.id !== rowId)
			...
			<View style={{ marginVertical: 8, maxWidth: 120, margin: 'auto' }}>
				...
				>
					...
					<Text style={{ textAlign: 'center' }}>
						<Text> Ordenes: </Text>
						<Text style={{ fontWeight: 'bold' }}>{selectedOrders?.length || 0} </Text>
					...
					{doneMessage && (
						<>
							...
							<Text style={[gStyles.helper, gStyles.tCenter]}>Cerrando ...</Text>
						</>
					)}
				</ButtonConfirm>
			</View>
