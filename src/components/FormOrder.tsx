         ...
         */}
				...
				>
					{({ handleSubmit, setValues, values, errors, setErrors, submitForm }) => {
						// FIXME: this can be causing performance issues
						// biome-ignore lint/correctness/useHookAtTopLevel: ill fix it after
						useEffect(() => {
							setErrors({})
						}, [values])
							...
							<>
								...
								{values.type === order_type.REPAIR && (
									<FormikInputSelect
										...
										options={Object.entries(repair_order_types).map(([_, value]) => ({
...
const FormFieldsA = ({ fields, values, setValues, setLoading }: FormFieldsProps) => {
	...
	const inputFields: Partial<Record<FormOrderFields, ReactNode>> = {
		...
		type: <InputRadiosFormik name="type" options={ordersTypesAllowed} label="Tipo de orden" />,
		sheetRow: (
			<InputTextStyled
