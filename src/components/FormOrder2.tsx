...
export const FormOrder2 = ({ onSubmit, defaultValues, title }: FormOrder2Props) => {
	...
	const { shop } = useShop()
	const defaultOrderType = shop?.orderTypes.RENT ? order_type.RENT : order_type.REPAIR
			...
			<View style={[gStyles.container]}>
				<Formik
					...
					validate={(values: Partial<OrderType>) => {
						...
						const ITEMS_MAX_BY_ORDER = shop?.orderFields?.[values.type]?.itemsMax
						const ITEMS_MIN_BY_ORDER = shop?.orderFields?.[values.type]?.itemsMin

						//* <- Validate items quantity if is required
						const VALIDATE_ITEMS_QTY =
							shop?.orderFields?.[values.type]?.validateItemsQty && values.hasDelivered
						/* ********************************************
						...
						return (
							<View>
								...
								{isCustomerSet && (
									<>
										<ViewInputForm>
											<CustomerOrderE customerId={customerId} canViewActions={false} />
										</ViewInputForm>
										<ViewInputForm>
											<Button
												size="xs"
											...
											<Text style={[gStyles.helper, gStyles.tCenter, { opacity: 0.7 }]}>
												* Al omitir cliente se borraran los datos y se podra crear una orden sin
												cliente.
											</Text>
										</ViewInputForm>
									</>
