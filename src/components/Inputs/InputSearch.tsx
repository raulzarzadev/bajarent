...
const InputSearch = <T extends { id: string | number }>({
	...
	useEffect(() => {
		...
		if (value.trim()) {
			const filtered = suggestions
				.filter(item => item[labelKey].toString()?.toLowerCase().includes(value.toLowerCase()))
	...
	const renderSuggestionItem = ({ item }: { item: T }) => {
		...
		return (
			<Pressable
				style={[styles.suggestionItem, isHovered && styles.suggestionItemHovered, { opacity: 0.7 }]}
		...
		<View style={[styles.container, style]}>
			<View style={styles.inputContainer}>
				<InputTextStyled
					...
					leftIcon={loading ? 'loading' : (value.length || value.length) === 0 ? 'search' : 'close'}
