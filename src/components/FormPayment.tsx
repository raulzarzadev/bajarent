...
const FormPayment = ({
	...
	onSubmit: (values: Partial<PaymentType>) => Promise<any> | undefined
	...
	const methods = Object.values(payment_methods).map(method => ({
		label: dictionary(method).charAt(0).toUpperCase() + dictionary(method).slice(1),
		value: method
	}))
	const references = store?.bankAccounts?.map(account => {
		const accountValue = account.value || ''
		const ref = `${account?.label} ${accountValue.substring(accountValue.length - 4)}`
		return {
			label: ref,
			value: ref
		}
				...
				return (
					<>
						...
						<View style={styles.repairItemForm}>
							<Button
								disabled={
									submitting || isSubmitting || !!Object.keys(errors).length || disabledSubmit
								}
								onPress={async () => {
									handleSubmit()
								}}
