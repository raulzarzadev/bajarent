...
const TabsA = ({
	...
	const handleTabPress = id => {
		setSelectedTab(id)
		setItem(`tab-${tabId}`, id)
		// const visibleTabs = tabs.filter(({ show }) => show)
		const tabIndexSelected = visibleTabs?.findIndex(tab => (tab?.id || tab.title) === id)
	...
	return (
		<View style={styles.container}>
					...
					{showAddTab && (
						<Pressable
							...
							style={({ pressed }) => [
								styles.tabButton,
								{ minWidth: 40 },
								pressed ? { backgroundColor: '#ddd' } : { backgroundColor: theme.primary },
								{ flexDirection: 'row' }
							]}
						...
						>
							...
							>
								{addTabTitle && <Text style={{ color: theme.white }}>{addTabTitle} </Text>}
					...
					{visibleTabs.map(tab => {
						...
						return (
							<Pressable
								...
								style={({ pressed }) => [
									styles.tabButton,
									selectedTab === id && styles.selectedTab,
			...
			<View style={styles.tabContent}>
				{visibleTabs?.find(tab => (tab?.id || tab.title) === selectedTab)?.content}
			</View>
		</View>
	)
}
