...
const OrderBigStatus = () => {
	...
	const dateInfo = date => `${dateFormat(asDate(date), 'dd MMM yy')} (${fromNow(asDate(date))})`
	...
	return (
		<View style={{ marginVertical: 2, flex: 1 }}>
			...
			{isAuthorized && (
				<Badge color={'warning'} title={`Pedido ${dateInfo(authorizedAt)}`} icon="calendarTime" />
			)}
			{isPending && (
				<Badge
					color={'warning'}
			...
			{isPickedUp && (
				<Badge color={'black'} title={`Recogido ${dateInfo(pickedUpAt)}`} icon="truck" />
			)}
			{isRepaired && (
				<Badge
					color={'success'}
...
const Badge = ({
	...
	return (
		<View style={{ marginVertical: 4 }}>
			...
			{!!description && (
				<View>
					<Text style={{ textAlign: 'center', marginTop: 4 }}>{description}</Text>
				</View>
			)}
		</View>
	)
