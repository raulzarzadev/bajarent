...
const SectionDetails = ({ section }: { section: SectionType }) => {
	...
	const staffSection = section?.staff?.map(staffId => staff?.find(s => s?.id === staffId))
...
export const AddStaffToThisSection = ({ sectionId }: { sectionId: string }) => {
	...
	const staffInSection = staff.filter(s => s.sectionsAssigned?.includes(sectionId))
	...
	const handleConfirmAddStaffToSection = async (sectionId: string, staffIds: string[]) => {
		setLoading(true)
	...
	return (
		<View style={[{ marginVertical: 14, marginHorizontal: 'auto' }, gStyles.container]}>
			<Button
				label="Agregar staff "
				icon="profileAdd"
				onPress={() => {
