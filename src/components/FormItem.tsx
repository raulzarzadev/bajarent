...
const FormItem = ({
	...
	const itemStatusOptions = Object.values(itemStatuses)?.map(status => ({
		label: status === 'pickedUp' ? 'Recogido/Disponible' : asCapitalize(dictionary(status)),
		value: status
	}))
	const canEditItemStatus =
		permissions.isAdmin || permissions.isOwner || permissions.items.canCreate || fromOrder
	return (
		<Formik
		...
		>
			{({ handleSubmit }) => (
				<View>
					{fromOrder && (
						<TextInfo defaultVisible text="Estos datos se escriben de forma automática" />
					)}
					<View
						style={{
					...
					<View style={styles.input}>
						<FormikInputValue label="No. de serie" name={'serial'} placeholder="No. serie" />
					</View>
					<View style={styles.input}>
						<FormikInputValue label="Marca" name={'brand'} placeholder="Marca" />
					</View>
					<View style={styles.input}>
						<FormikInputSelect
							label="Categoria"
