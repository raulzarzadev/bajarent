...
const BalanceItemsTable = ({ balance }: BalanceItemsTableProps) => {
	...
	return (
		<View>
			...
			>
				<ExpandibleBalanceItems items={createdItems} label="Articulos creados" />
				<ExpandibleBalanceItems items={retiredItems} label="Articulos retirados" />
			</View>
			...
			{Object.keys(groupedBySection)
				?.sort((a, b) => {
					const aName = storeSections?.find(s => s.id === a)?.name || 'Sin asignar'
					const bName = storeSections?.find(s => s.id === b)?.name || 'Sin asignar'
				...
				.map((balanceRow: string) => {
					...
					const currentlyAvailable = balanceRowItems.filter(i => !i?.retiredAt)
					...
					return (
						<View key={balanceRow}>
							...
							<View
								style={{
									backgroundColor: selectedRow === balanceRow ? 'lightblue' : 'transparent'
								}}
							>
												...
												component: (
													<View>
														<Text style={gStyles.tCenter}>
															...
															{field === 'allItems' && currentlyAvailable.length}
														</Text>
													</View>
												),
												width
								...
								{selectedRow === balanceRow && (
									<View>
										...
										>
											<ExpandibleBalanceItems items={inStock} label="Disponibles" />
											<ExpandibleBalanceItems items={inRent} label="Ordenes con items" />
										</View>
									</View>
								)}
							</View>
...
const TABLE: {
	field: keyof BalanceRowType | 'allItems' | 'newRouteItems' | 'newStoreItems' | 'retired'
	label: string
