...
const RentsBalance = ({ balance }: RentsBalanceProps) => {
	...
	const rents = balance?.orders?.filter(order => order?.orderType === order_type.RENT)
	//* group by section

	const groupedBySections = rents?.reduce(
		(acc, order) => {
	...
	const sectionsInfo = Object.keys(groupedBySections).map(sectionId => {
		return {
			sectionId,
			sectionName: storeSections?.find(section => section.id === sectionId)?.name || 'Sin asignar'
		}
	})
