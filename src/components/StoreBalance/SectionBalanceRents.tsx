...
const SectionBalanceRents = ({
	...
	const actives = orders?.filter(order => order?.orderStatus === order_status.DELIVERED)
	const pickedUp = orders?.filter(order => order?.orderStatus === order_status.PICKED_UP)
	...
	const formatItemsWithSectionAndCategoryName = item => ({
		...
		categoryName:
			item?.categoryName || categories.find(category => category.id === item.categoryId)?.name
	})
	...
	return (
		<View>
			...
			>
				<ExpandibleBalanceOrders orders={delivered} label="Rentas" defaultExpanded />
				<ExpandibleBalanceOrders orders={renewed} label="Renovadas" defaultExpanded />
				<ExpandibleBalanceOrders orders={pickedUp} label="Recogidas" defaultExpanded />
				<ExpandibleBalanceOrders orders={extended} label="Extendidas" defaultExpanded />
			</View>
			<View style={{ flexDirection: 'row', justifyContent: 'space-evenly' }}>
			...
			<View style={{ flexDirection: 'row', justifyContent: 'space-evenly' }}>
				...
				<ExpandibleBalanceOrders orders={solvedReports} label="Reportes resueltos" />
			</View>
		</View>
	)
}
