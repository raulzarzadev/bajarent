...
export const BalanceView = ({ balance }: BalanceViewProps) => {
	...
	const balanceContainRepairs = balance.orders.some(order => order.orderType === order_type.REPAIR)
	const balanceContainSales = balance.orders.some(order => order.orderType === order_type.SALE)
			...
			<Tabs
				...
				tabs={[
					...
					},
					{
						...
						show: balanceContainRepairs
						//disabled: true
					},
					{
						title: 'Ventas',
						content: <SalesBalanceE balance={balance} />,
						show: balanceContainSales
						// disabled: true
					}
				]}
			/>
...
const generateBalanceCSV = (
	...
	let csv = ''
	csv += `Período ${JUM} ${formatDate(balance.fromDate)} - ${formatDate(balance.toDate)}${JUM}`
