...
const OrderStatus = ({
	...
	const expireLabel = expireToday ? 'hoy' : `${fromNow(asDate(order?.expireAt))}`
	...
	const isRenewed =
		order.type === 'RENT' && (order.isRenewed || order?.status === order_status.RENEWED)

	const NullExpireAt =
		!order?.expireAt && order.type === 'RENT' && order.status === order_status.DELIVERED
	...
	return (
		<>
			{isRenewed && (
				<Chip style={[chipStyles]} title={'Renovada'} color={theme.transparent} size={chipSize} />
			)}
			{isCancelled && (
				<Chip
