	...
	const fields: ListRowField[] = [
		{
			component: (
				<View>
					<Text style={{ fontWeight: 'bold', fontSize: 16 }}>{staff?.name}</Text>
					{!!staff?.position && (
						<Text style={{ fontSize: 12, color: '#666' }}>{staff?.position}</Text>
					)}
					{!!staff?.email && <Text style={{ fontSize: 10, color: '#999' }}>{staff?.email}</Text>}
				</View>
			),
			width: '40%'
		},
