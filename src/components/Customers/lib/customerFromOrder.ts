...
export const customerFromOrder = (
	...
	const contactsList = Object.values(customerContacts || {}).map(contact => contact.value || '')
...
export const findSimilarCustomer = (
	...
	return (
		customers.find(c => {
			...
			const storeCustomerPhoneNumbers = Object.values(c.contacts).map(contact => contact.value)
			const newCustomerPhoneNumbers = Object.values(customer.contacts).map(contact => contact.value)
			return storeCustomerPhoneNumbers.find(sPhone => newCustomerPhoneNumbers.includes(sPhone))
		}) || null
	)
}
...
export const mergeCustomers = (
	...
	const name =
		customerName1.includes(customerName2) || customerName2.includes(customerName1)
			? customerName1
...
const mergeImages = (images1: CustomerImages = {}, images2: CustomerImages = {}) => {
...
const mergeContacts = (
	...
	const existingValues = Object.values(contacts1).map(contact => contact.value)
