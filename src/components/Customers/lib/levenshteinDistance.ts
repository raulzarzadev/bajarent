...
export function findBestMatch(customers: string[], query: string) {
	...
	for (const customer of customers) {
		const distance = levenshteinDistance(customer.toLowerCase(), query.toLowerCase())
...
export function findBestMatches(data: string[], query: string, count?: number) {
	...
	const matches = data.map(item => {
		...
		const { distance, matchBonus } = levenshteinDistanceExtended(lowerCustomer, lowerQuery)
	...
	const filteredMatches = sortedMatches.filter(
		match => match.keywordMatches === maxKeywords && match.matchBonus === maxBonus
	)
