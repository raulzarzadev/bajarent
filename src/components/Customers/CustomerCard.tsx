...
const CustomerCard = (props?: CustomerCardProps) => {
	...
	const canRead = customerId && (permissions?.customers?.read || permissions.isAdmin)
	const canEdit = customerId && (permissions?.customers?.edit || permissions.isAdmin)
	const canDelete = customerId && (permissions?.customers?.delete || permissions.isAdmin)
	...
	return (
		<View style={{ justifyContent: 'center' }}>
			...
			<Text style={{ textAlign: 'center' }}>Colonia: {customer?.address?.neighborhood}</Text>
			<Text style={{ textAlign: 'center' }}>Calle: {customer?.address?.street}</Text>
			<Text style={{ textAlign: 'center' }}>Referencias: {customer?.address?.references}</Text>
			<View
				style={{
					flexDirection: 'row',
			...
			<CustomerContactsE customerId={customer.id} canAdd customerContacts={customer.contacts} />
			<CustomerImagesE images={customer?.images} customerId={customer?.id} canAdd />
		</View>
	)
}
