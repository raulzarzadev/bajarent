	...
	if (storesStatus === 'error') {
		return (
			<View style={styles.container}>
				...
				<Text style={[gStyles.helper, { textAlign: 'center', marginVertical: 8 }]}>
					{storesError || 'No pudimos obtener tus tiendas. Inténtalo de nuevo.'}
				</Text>
				<Button label="Reintentar" onPress={() => user?.id && handleSetUserStores(user.id)} />
			</View>
		)
	}
				...
				<FlatList
					...
					renderItem={({ item }) => (
						...
						>
							<Text
								numberOfLines={2}
								style={[gStyles.h3, { color: theme.black, textAlign: 'center' }]}
							>
								{item.name}
							</Text>
							{!!item.description && (
								<Text style={[gStyles.helper, { marginTop: 4, textAlign: 'center' }]}>
									{item.description}
								</Text>
							)}
						</Pressable>
			...
			<Modal visible={showCreateForm} animationType="slide">
				<View style={{ flex: 1 }}>
					<View style={{ padding: 16 }}>
						<Button variant="ghost" label="Cerrar" onPress={() => setShowCreateForm(false)} />
					</View>
					<ScreenStoreCreate navigation={{ goBack: () => setShowCreateForm(false) }} />
				</View>
			</Modal>
		</View>
	)
