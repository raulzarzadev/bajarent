...
const InputMapLocation = ({
	...
	const isDirtyLocation = JSON.stringify(mapCenter) !== JSON.stringify(defaultCoords)
	...
	return (
		<View style={{ position: 'relative', width: '100%' }}>
			...
			<View style={{ zIndex: 99999999 }}>
				...
				{responses?.length === 0 && <Text>No se encontraron coincidencias</Text>}
				<View
					style={{
						position: 'absolute',
						top: 0,
					...
					{showResponses &&
						responses?.map((response: NominatimResult) => (
							<Pressable
								...
								onPress={() => {
									const newCenter: CoordsType = [parseFloat(response.lat), parseFloat(response.lon)]
									setCoords(newCenter)
									setShowResponses(false)
								}}
							>
	...
	const eventHandlers = useMemo(
		() => ({
			dragend() {
				...
				if (marker !== null) {
					const coords: CoordsType = [marker.getLatLng()?.lat, marker.getLatLng()?.lng]
					setPosition(coords)
					setCenter(coords)
				}
			}
...
function MapCenterTracker({ onCenterChange }: { onCenterChange: (center: CoordsType) => void }) {
	const map = useMap()
