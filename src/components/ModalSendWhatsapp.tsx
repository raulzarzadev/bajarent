...
export default function ModalSendWhatsapp({
	...
	//*********  MEMES
	const WELCOME = `Estimado ${customer?.name || order?.fullName || ''} cliente de ${store?.name}`
...
`
  ...
  🔚 Vencimiento: ${dateFormat(asDate(orderExpiresAt), 'EEEE dd MMMM yy')}`
	...
	const expireDateString = order => {
		...
		if (isAfterTomorrow(date)) {
			return `Su contrato VENCE EL ${dateFormat(date, 'EEEE dd MMMM yy')} (${fromNow(date)})`
		}
	...
	const SOCIALS = store?.socialMedia?.map(s => `${s?.type || ''}: ${s?.value || ''}`)?.join('\n')
  ...
  \n${getReceiptDates(order)}`
	...
	const itemFailure =
		order?.type === order_type.REPAIR && (order?.item?.failDescription || order?.itemFailure || '')
	...
	return (
		<View>
			...
			>
				...
				{invalidPhone && (
					<Text style={[gStyles.helperError, { textAlign: 'center', marginVertical: 6 }]}>
...
export const ButtonSendWhatsappStatatus = (props: ButtonSendWhatsappStatatusProps) => {
	...
	return (
		...
		>
			<TextInfo text="Se enviara el siguiente mensaje" defaultVisible type="info"></TextInfo>
			<Text>{message}</Text>
		</ButtonConfirm>
	)
}
...
export const ButtonSendWhatsappStatatusE = (props: ButtonSendWhatsappStatatusProps) => (
	<ErrorBoundary componentName="ButtonSendWhatsappStatatus">
		<ButtonSendWhatsappStatatus {...props} />
	</ErrorBoundary>
)

const dFormat = (date: Date) => (date ? dateFormat(asDate(date), 'dd/MMM/yy HH:mm') : '')
...
const getReceiptDates = (order: OrderType): string => {
	...
	if (isRent) {
		...
		if (isDelivered && order.deliveredAt) dates.push(`Entregada ${dFormat(order?.deliveredAt)}`)
...
const orderStringDates = (order: Partial<OrderType>, format = 'EEEE dd MMMM yy') => {
	if (order?.extensions) {
		const lastExtension = Object.values(order?.extensions || {})?.sort((a, b) => {
