...
const sanitizePayload = <T extends Record<string, unknown>>(data: T): SanitizerResult<T> => {
	const mutatedPaths: string[] = []
	const sanitized = sanitizeValue(data, '', mutatedPaths)
	return { sanitized, mutatedPaths }
}

export async function firebaseSafeSave<T extends Record<string, unknown>, R = void>({
	data,
	saveFn,
	contextLabel
}: FirebaseSafeSaveParams<T, R>): Promise<R> {
