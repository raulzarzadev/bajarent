...
export type PermissionsStore = Partial<Record<keyof typeof permissions_store, boolean>>
export type PermissionsItems = Partial<Record<keyof typeof permissions_items, boolean>>

export type PermissionsCustomers = Partial<Record<keyof typeof permissions_customers, boolean>>
...
export const staffPermissions = {
	isOwner: 'isOwner',
	isAdmin: 'isAdmin',
	order: Object.keys(permissions_orders).reduce(
		(acc, key) => {
			acc[key] = key
			return acc
		},
	...
	store: Object.keys(permissions_store).reduce(
		(acc, key) => {
			acc[key] = key
			return acc
		},
	...
	items: Object.keys(permissions_items).reduce(
		(acc, key) => {
			acc[key] = key
			return acc
		},
	...
	customers: Object.keys(permissions_customers).reduce(
		(acc, key) => {
			acc[key] = key
			return acc
		},
