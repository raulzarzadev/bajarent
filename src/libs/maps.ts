...
export const getCoordinatesAsString = (coords: CoordsType): CoordsStringType => {
...
export const formatOrderItemsMaps = async (orders: OrderType[]): Promise<ItemMap[]> => {
	return await Promise.all(
		orders
			?.map(async order => {
...
export const createItemsFromOrder = (order: OrderType): ItemBase[] => {
	const isRented = order.status === order_status.DELIVERED && order.type === order_type.RENT
