...
}: //orders
{
   ...
   *******************************************rz */
	...
	const filterOrdersByUser = (orders: any[], values: BalanceType, field: string) => {
		return (
			orders
				.filter(o => {
...
export const calculateSectionBalance = async ({
	...
	const ordersIdsWithSolvedReports = Array.from(new Set(reportsUnsolved.map(r => r.orderId)))

	const ordersWithSolvedReports = await ServiceOrders.getList(ordersIdsWithSolvedReports)
	...
	const ordersRenewed = ordersInRent?.filter(o => {
		const renews = Object.values(o.extensions || {}).filter(e => e.reason === 'renew')
		// .sort((a, b) =>
