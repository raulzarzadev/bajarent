...
export const formatOrder = ({
	...
	formattedOrder.pendingMarketOrder =
		formattedOrder?.status === order_status.PENDING && formattedOrder.marketOrder
	...
	if (formattedOrder?.type === 'RENT') {
		...
		const isExpired = !!expireAt && (isBefore(asDate(expireAt), new Date()) || expireToday)
		...
		return {
			...
			expiresOnMonday: isSaturday(new Date()) && isMonday(asDate(expireAt)) && !isExpired
		}
	}
...
export const isUnsolvedOrder = (order: OrderType) => {
	...
	if ([order_status.CANCELLED, order_status.RENEWED].includes(order.status)) return false
...
export const orderExpireAt = ({ order }: { order: Partial<OrderType> }): Date | null => {
	let expireAt = null
...
export const filterActiveOrders = (orders: OrderType[]) => {
	return orders.filter(o => {
		...
		if (o.type === 'RENT') {
			return [order_status.AUTHORIZED, order_status.DELIVERED].includes(o.status)
		}
		if (o.type === 'SALE') {
...
export const isRenewedToday = (order: Partial<OrderType>): boolean => {
	const lastExtension = Object.values(order?.extensions || {})
		...
		.sort((a, b) => asDate(b?.createdAt)?.getTime() - asDate(a?.createdAt)?.getTime())[0]
...
export const excelFormatToOrder = (
	...
	OrderType,
	'note' | 'fullName' | 'phone' | 'neighborhood' | 'address' | 'references' | 'scheduledAt'
> => {
