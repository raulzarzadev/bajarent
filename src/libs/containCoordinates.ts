...
const containCoordinates = (
	...
	const coords: CoordsType | null = match ? [parseFloat(match[1]), parseFloat(match[2])] : null

	// Verificar si la cadena coincide con alguna de las expresiones regulares
	return {
		containCoords:
			regex1.test(input) || regex2.test(input) || regex3.test(input) || regex4.test(input),
		coords
	}
}
