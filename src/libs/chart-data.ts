...
export const groupDocsByMonth = <T extends { createdAt: Date | Timestamp }>({
	...
	const groupedOrdersByMonth: { [key: string]: ChartDocType<T>[] } = docs.reduce((acc, doc) => {
...
export const groupDocsByWeek = <T extends { createdAt: Date | Timestamp }>({
	...
	const groupedOrdersByWeek: { [key: string]: ChartDocType<T>[] } = docs.reduce((acc, doc) => {
...
export const groupDocsByDay = <T extends { createdAt: Date | Timestamp }>({
	...
	const groupedOrdersByDay: { [key: string]: ChartDocType<T>[] } = docs.reduce((acc, doc) => {
		const date = asDate(doc.createdAt)
...
export const groupDocsByType = <T extends { type: string }>({ docs }: { docs: T[] }) => {
