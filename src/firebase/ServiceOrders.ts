...
class ServiceOrdersClass extends FirebaseGenericService<Type> {
	...
	storeOrders(storeId: string, cb: CallableFunction) {
	...
	async addComment({
		...
		return await ServiceComments.create(comment).then(console.log).catch(console.error)
	}
	...
	async updateComment(commentId, updates) {
		return await ServiceComments.update(commentId, updates).then(console.log).catch(console.error)
	}
	...
	listenUnsolved(storeId: string, cb: CallableFunction) {
		return this.listenMany(
			[where('storeId', '==', storeId), where('status', 'in', ORDER_STATUS_UNSOLVED)],
			cb
		)
	}
	...
	async getOrderExpiresOnDate({ date, storeId }: { date: Date; storeId: string }) {
	...
	async getOrdersScheduledAt({ date, storeId }: { date: Date; storeId: string }) {
		const start = startOfDay(date)
		const end = endOfDay(date)
		return await this.findMany([
			where('storeId', '==', storeId),
	...
	getList(ids: string[], ops: GetItemsOps & { sections?: string[] } = {}): Promise<Type[]> {
		const maxAllowedIds =
			ops.sections && ops.sections.length > 0 ? Math.floor(30 / ops.sections.length) : 30
	...
	getPending(storeId: string, ops: { sections?: string[] } = {}) {
		...
		if (sections?.length > 0) filters.push(where('assignToSection', 'in', sections))
		return this.findMany(filters)
	}
	...
	getRepairsUnsolved(storeId: string, ops: { sections?: string[] } = {}) {
		...
		const filters = [
			...
			where('status', 'in', [order_status.REPAIRING, order_status.REPAIRED, order_status.PICKED_UP])
		]
		if (sections?.length > 0) filters.push(where('assignToSection', 'in', sections))
		return this.findMany(filters)
	}
	...
	async getOrdersUpdatedDate({ date, storeId }: { date: Date; storeId: string }) {
		return this.findMany([where('storeId', '==', storeId), where('updatedAt', '>=', date)])
	}
		...
		const promises = fields
			?.map(field => {
				...
				if (field === 'folio') {
					...
					return itsValidNumber ? this.findMany([...filters, where(field, '==', asNumber)]) : null
				}
	...
	async getUnsolvedByStore(
		...
		const ordersWithReportsAndImportantUnsolved = Array.from(
			new Set(
				reports
					?.filter(
						({ type, solved }) => (type === 'report' || type === 'important') && solved === false
					)
					?.map(({ orderId }) => orderId)
			)
		)
		...
		if (ordersWithReportsAndImportantUnsolved.length > 0) {
			...
			if (res) {
				...
				console.log('no ordersWithReportsAndImportantUnsolved in reported orders')
			}
		}
		...
		const removeReportedFromUnsolved = unsolvedOrders.filter(
			({ id }) => !reportedOrders?.find(({ id: reportedId }) => reportedId === id)
		)
	...
	async getClientOrders({ clientId, storeId }, ops?: { justIds: boolean }) {
		...
		if (justIds)
			return this.findMany([where('clientId', '==', clientId), where('storeId', '==', storeId)], {
				justRefs: true
			})
	...
	static createFilterToGetOrderFieldValue = ({
		...
		const filters = [where('storeId', '==', storeId), where('status', '==', status)]
