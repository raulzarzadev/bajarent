...
class ServiceBalancesClass extends FirebaseGenericService<BalanceType2> {
	...
	async listenLastInDate(storeId: string, date: Date, cb: (data: Partial<BalanceType2>[]) => void) {
...
const groupOrdersBySection = ({
	...
	const sections = Object.entries(groupedSections).map(
		([sectionId, orders]: [string, Partial<OrderType>[]]) => {
			...
			let inStock = items
				.filter(item => sectionId === (item.assignedSection || WITHOUT_SECTION_NAME))
				.map(({ id }) => id)
			if (sectionId === 'all') {
				inStock = items.map(({ id }) => id)
			}

			const pending = orders?.filter(order => order.status === order_status.AUTHORIZED)
			const inRent = orders?.filter(order => order.status === order_status.DELIVERED)
			const reported = orders?.filter(order => reports.some(r => r.orderId === order.id))
			...
			const deliveredToday = orders?.filter(
				order => order.status === order_status.DELIVERED && isToday(asDate(order.deliveredAt))
			)
			const pickedUpToday = orders?.filter(
				order => order.status === order_status.PICKED_UP && isToday(asDate(order.pickedUpAt))
			)

			const renewedToday = orders.filter(
				order => order.status === order_status.DELIVERED && isRenewedToday(order)
			)

			const renewsToday = getTodayRenews({ orders })
			const cancelledToday = orders.filter(
				order => order.status === order_status.CANCELLED && isToday(asDate(order?.cancelledAt))
			)
			...
			if (sectionId === 'all') {
				...
				sectionPayments = payments.filter(
					p => orders.find(o => o.id === p.orderId) || p.sectionId === sectionId
				)
			}
			...
			return {
				...
				reported: filterOutElements(getJustIds(reported), getJustIds(solvedToday)),
