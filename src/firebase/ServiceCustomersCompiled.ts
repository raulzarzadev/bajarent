...
class ServiceCustomersCompiledClass extends FirebaseGenericService<{
	...
	addCustomer = async (storeId: string, customerId: string, customer: CustomerCompiledType) => {
	 ...
	 * @returns
	 */
	async updateCustomer(storeId: string, customerId: string, customer: CustomerCompiledType) {
		return this.update(storeId, {
			[`customers.${customerId}`]: customer
		})
	}
