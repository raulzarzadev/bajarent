...
export class FirebaseCRUD {
	...
	async updateItem(itemId: string, item: object) {
		...
		return await updateDoc(doc(this.db, this.collectionName, itemId), newItem).then(res =>
	 ...
	 */
	async getItem(itemId: string, ops?: GetItemsOps) {
	...
	async updateItemInCollection({
		...
		return await updateDoc(doc(this.db, parentCollection, parentId, subCollection, itemId), newItem)
			.then(res =>
				this.formatResponse(true, `${this.collectionName}_UPDATED`, {
					id: itemId
				})
	...
	async deleteItemInCollection({
		...
		return deleteDoc(doc(this.db, parentCollection, parentId, subCollection, itemId))
			.then(res =>
	...
	async updateFieldInSubCollection({
		...
		const ref = doc(this.db, this.collectionName, parentId, subCollection, itemId)
	...
	// }
	showSnapshot(
		querySnapshot: QuerySnapshot<DocumentData, DocumentData> | DocumentSnapshot<DocumentData>,
		...
		const filters = querySnapshot.query._query.filters.map(f => {
			...
			const booleanValue =
				typeof f?.value?.booleanValue === 'boolean' ? `${f.value.booleanValue}` : null

			const arrayValues = f?.value?.arrayValue?.values?.map(v => v?.stringValue)?.join(', ') || null
