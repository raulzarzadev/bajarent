...
export const onCreateItem = async ({ storeId, item }: { storeId: string; item: Type }) => {
...
export const onDeleteItem = async ({ storeId, itemId }: { storeId: string; itemId: string }) => {
...
export const onChangeOrderItem = async ({
	...
	try {
		if (!newItemId || itemId === newItemId) return console.log('no new item or is the same ')
		//* get numbers
		const oldItem = await ServiceStoreItems.get({ storeId, itemId }).then(res => res)
		...
		onComment({
			...
			storeId,
			orderId,
			content: `Se cambio el artículo ${oldItemNumber || 'SA'} por el artículo ${newItemNumber}`
		})
	} catch (e) {
