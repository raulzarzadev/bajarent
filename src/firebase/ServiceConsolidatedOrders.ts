...
class ConsolidatedOrdersClass extends FirebaseGenericService<Type> {
	...
	async getByStore(storeId: string) {
		return this.findMany([where('storeId', '==', storeId), orderBy('createdAt', 'desc'), limit(1)])
	}
			...
			return ServiceChunks.create({
				...
				const progressValue =
					chunks && chunks.length > 1
						? 100 * CHUNK_AVERAGE_CHARGE + (i / (chunks.length - 1)) * 100 * CHUNK_AVERAGE_CHARGE
...
const formatConsolidateOrders = (orders: OrderType[], payments: PaymentType[]) => {
