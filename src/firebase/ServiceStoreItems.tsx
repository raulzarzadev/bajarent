...
export class ServiceStoreItemsClass {
	async add({ storeId, item }: { storeId: string; item: Type }): Promise<{
		...
		//* SECOND CREATE ITEM
		const nextNumber = (await ServiceStores.get(storeId)).currentEco as unknown as number
	...
	async getAll(
		{ storeId, sections, justActive }: { storeId: string; sections?: string[]; justActive?: true },
		ops?: GetItemsOps
	): Promise<Type[]> {
		const filters = []
		if (sections?.length) {
		...
		if (justActive) {
			filters.push(where('status', 'in', [ItemStatuses.rented, ItemStatuses.pickedUp]))
		}

		const storeItemsRef = ServiceStores.getSubCollectionRef(storeId, SUB_COLLECTION)
	...
	listenAvailableBySections({
		...
		if (Array.isArray(userSections) && userSections.length > 0) {
			return ServiceStores.listenItemsInSubCollection({
				parentId: storeId,
				subCollection: SUB_COLLECTION,
				filters: [where('status', '==', 'pickedUp'), where('assignedSection', 'in', userSections)],
				cb
			})
		}
		if (userSections === 'all') {
