...
export class ServiceStoresClass extends FirebaseGenericService<StoreType> {
	...
	async removeOldPermissions({ storeId, staffId }: { storeId: string; staffId: string }) {
		const store = await this.get(storeId)
		if (!store) throw new Error('Store not found')
		const currentStaff = store.staff || []
	...
	async removeStaff({ storeId, staffId }: { storeId: string; staffId: string }) {
		...
		if (userIdToRemove) {
			updatedStaffUserIds = updatedStaffUserIds.filter(id => id !== userIdToRemove)
		}
	...
	async updateStaffSectionsAssigned({
		...
		if (!store) throw new Error('Store not found')
		const currentStaff = store.staff || []
		const updatedStaff = currentStaff.map(s => (s.id === staffId ? { ...s, sectionsAssigned } : s))
		return this.update(storeId, { staff: updatedStaff })
	}
