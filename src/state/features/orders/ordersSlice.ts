...
export const fetchUnsolvedOrders = createAsyncThunk(
	...
	async (
		...
		try {
			...
			const reports = await ServiceComments.getUnsolvedImportantAndReports(params.storeId)
...
const categorizeOrders = (orders: Partial<OrderType>[], sections: string[] = []) => {
	const categories = {
		unsolved: [] as string[],
		solved: [] as string[],
		my: [] as string[],
	...
	orders.forEach(order => {
		...
		if (sections.length > 0 && order.assignToSection && sections.includes(order.assignToSection)) {
...
const ordersSlice = createSlice({
	...
	reducers: {
		...
		removeListener: (state, action: PayloadAction<string>) => {
			state.listeners = state.listeners.filter(listener => listener !== action.payload)
		}
	},
...
export const selectOrdersByType = (state: { orders: OrdersState }, type: string) => {
...
export const selectReports = (state: { orders: OrdersState }) => state.orders.reports
