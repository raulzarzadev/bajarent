...
export const fetchCustomers = createAsyncThunk(
	...
	async ({
		...
		if (readAll) {
			...
			console.log(`read on demand ${onDemandList?.length} customers`)
			response = await ServiceCustomers.listIds({ ids: onDemandList })
		}

		const customers = response.map(customer => convertTimestamps(customer, { to: 'string' }))
		return customers
	}
)
...
export const updateCustomer = createAsyncThunk(
	...
	async ({
		...
		if (customer?.contacts) {
			customer.contactsList = Object.values(customer.contacts).map(contact => contact.value)
		}
