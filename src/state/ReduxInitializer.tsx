...
import { type FetchTypeOrders, useOrdersCtx } from '../contexts/ordersContext'
...
import { fetchOrdersByType, setStoreConfig } from './features/orders/ordersSlice'
...
export const ReduxInitializer = ({ children }) => {
	...
	useEffect(() => {
		const uniqueCustomerIds = Array.from(new Set(orders?.map(o => o.customerId).filter(id => id)))
...
const useInitializeOrdersState = () => {
	...
	}, [dispatch, storeId, employee?.sectionsAssigned, permissions?.canViewAllOrders])
	...
	useEffect(() => {
		if (
			storeId &&
			!disabledEmployee &&
			(permissions?.isAdmin || permissions?.orders?.canViewMy || permissions?.canViewAllOrders)
		) {
			const fetchType: FetchTypeOrders = permissions?.canViewAllOrders ? 'all' : 'mine'
