...
class OrdersSyncService {
	...
	private setupOrdersListener() {
		...
		try {
			...
			const listenerResult = ServiceOrders.listenUnsolved(this.storeId, (orders: OrderType[]) => {
				if (this.dispatch) {
			...
			if (listenerResult && typeof (listenerResult as any).then === 'function') {
	...
	private setupCommentsListener() {
		...
		try {
			...
			const pollComments = async () => {
				try {
					const reports = await ServiceComments.getUnsolvedImportantAndReports(this.storeId!)
					if (this.dispatch) {
	...
	// Health check
	isHealthy(): boolean {
		return this.isListening && this.listeners.size > 0 && !!this.dispatch && !!this.storeId
	}
...
export const useOrdersSync = (dispatch: AppDispatch, storeId: string | null) => {
	const startSync = () => {
		if (storeId && dispatch) {
