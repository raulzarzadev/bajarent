 ...
 */
export function filterByCustomIds<T extends { id?: string }>(data: T[], ids: string[]): T[] {
...
export function handleFilterUpdate<T>(
	...
	const sameExist = currentFilters.some(a => a.field === field && a.value === value)
	...
	if (sameExist) {
		const cleanedFilters = currentFilters.filter(a => !(a.field === field && a.value === value))
...
export function searchInLocalData<T extends { id?: string }>(
	...
	const filteredMatchedData =
		filters.length > 0 ? filterDataByFields(matchedItemData, filters) : matchedItemData
