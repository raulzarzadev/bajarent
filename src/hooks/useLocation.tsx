	...
	const getLocation = async (): Promise<LocationRes | null> => {
		...
		try {
			...
			const timeoutPromise = new Promise(
				(_, reject) => setTimeout(() => reject(new Error('Permission request timed out')), 10000) // 10 seconds timeout
			)

			const result = (await Promise.race([permissionPromise, timeoutPromise])) as {
				status: PermissionState
			}
			const { status } = result
			console.log({ status })
